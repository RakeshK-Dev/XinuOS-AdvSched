...
void burst_execution(uint32 number_bursts, uint32 burst_duration, uint32 sleep_duration)
{
    ...
    for (count = 1; count < number_bursts + 1; count++)
    {
        // Busy wait until the process has executed for the desired duration
        while (proctab[currpid].runtime < burst_duration * count);

        // Sleep for the specified duration before the next burst
